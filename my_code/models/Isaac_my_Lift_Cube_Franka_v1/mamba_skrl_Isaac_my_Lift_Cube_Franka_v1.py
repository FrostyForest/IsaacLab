...
class SharedMamba(GaussianMixin, DeterministicMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        if self.training and not self.single_step:
            ...
            initial_cache_flat = initial_cache_flat[:, indices, :]
        ...
        if self.training and not self.single_step:
            ...
            for i in range(self.sequence_length):
                ...
                if terminated is not None and i > 0:
                    finished_episodes_mask = terminated[:, i - 1].view(-1, 1, 1)  # Reshape for broadcasting

                    for h_cache, conv_cache in current_caches:
                        # Use torch.where to create a new tensor without inplace modification
            ...
            mamba_output = torch.cat(outputs, dim=1)
            mamba_output = mamba_output.view(num_transitions, -1)
        else:
            # Rollout: step is defined on Mamba class for multi-layer processing
