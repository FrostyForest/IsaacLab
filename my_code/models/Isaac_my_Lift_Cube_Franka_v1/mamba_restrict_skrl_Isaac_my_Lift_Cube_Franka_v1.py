...
# 该模型对最不需要的模态进行了抑制，共15个模态，去除物体位置即14个模态
class SharedMamba(GaussianMixin, DeterministicMixin, Model):
    def __init__(
        ...
        self.observable_domains = [
            "joint_pos",
            "joint_vel",
            "previous_actions",  # 对应 space["actions"]
            # "object_pos", 不需要位置
            "ee_pos_ori",  # 将 ee_position 和 ee_camera_orientation 合并
            # "target_pos", 任务目标不需要block
            "contact_forces",  # 将左右指力合并
            "rgb_feature",
            "depth_feature",  # 对应处理后的 depth_obs
            # "text_clip_feature",任务目标不需要block
            "image_clip_feature",
        ]
        self.num_mask_options = len(self.observable_domains)

        # 为每个域创建一个可学习的 logit
        self.masking_logits = nn.Parameter(torch.zeros(len(self.observable_domains), device=device))
    ...
    def sample_masks_hard(self, batch_size):
        """
        Samples a batch of one-hot encoded masks.
        Returns the one-hot tensors, the sampled indices, and their log probabilities.
        """
        # 1. 创建分布
        masking_dist = torch.distributions.Categorical(logits=self.masking_logits)

        # 2. 采样索引
        sampled_indices = masking_dist.sample((batch_size,))

        # 3. 计算对数概率 (用于 REINFORCE 更新)
        log_probs = masking_dist.log_prob(sampled_indices)

        # 4. 转换为 One-Hot
        one_hot_masks = F.one_hot(sampled_indices, num_classes=len(self.observable_domains)).float()

        return one_hot_masks, sampled_indices, log_probs

    def sample_masks_gumbel(self, batch_size, tau=1.0, hard=False):
        """
        Samples a batch of masks using the Gumbel-Softmax trick.
        Args:
            batch_size (int): The number of masks to sample.
            tau (float): The temperature for the Gumbel-Softmax distribution.
            hard (bool): If True, the returned samples will be discrete one-hot vectors,
                         but gradients are still passed through the soft samples.
        Returns:
            torch.Tensor: A tensor of shape (batch_size, num_mask_options) containing
                          the sampled soft or hard masks.
        """
        min_tau = min(0.1, tau)
        max_tau = tau
        dist = torch.distributions.Categorical(logits=self.masking_logits)
        entropy = dist.entropy()  # 计算当前 logits 的熵
        # 将熵映射到温度 tau
        # 熵的范围是 [0, log(D)]，D 是类别数
        # 我们希望熵最大时 tau 最大，熵最小时 tau 最小
        max_entropy = torch.log(torch.tensor(self.num_mask_options, dtype=torch.float))

        # 简单的线性映射
        normalized_entropy = entropy / max_entropy
        current_tau = min_tau + (max_tau - min_tau) * normalized_entropy
        # 1. 准备 Logits
        # gumbel_softmax 需要 (batch_size, num_classes) 形状的 logits.
        # 我们的 self.masking_logits 是 (num_classes,)，需要扩展 (expand) 以匹配 batch_size.
        logits_batch = self.masking_logits.unsqueeze(0).expand(batch_size, -1)

        # 2. 使用 Gumbel-Softmax 进行采样
        # 这个函数直接返回一个类似 one-hot 的向量。
        # 这个向量是连续的、可微的，并且其梯度可以流回 logits_batch。
        gumbel_samples = F.gumbel_softmax(logits_batch, tau=current_tau, hard=hard)

        # 3. 返回结果
        # 我们不再需要返回 indices 和 log_probs，因为 Gumbel-Softmax 的输出
        # 本身就是我们要在后续计算中使用的张量，并且梯度流的问题已经解决了。
    ...
    def _extract_and_combine_features(self, space):
        ...
        batch_size = norm_joint_pos.shape[0]
        ...
        gumbel_masks = self.sample_masks_gumbel(batch_size=batch_size)  # logits越大，被屏蔽的概率越大
        # print('mask shape',gumbel_masks.shape)
        # print(gumbel_masks)
        masks = {}
        for i, domain in enumerate(self.observable_domains):
            # 乘子 = 1 - 屏蔽信号
            masks[domain] = (1.0 - gumbel_masks[:, i]).unsqueeze(-1)

        norm_joint_pos = norm_joint_pos * masks["joint_pos"]
        norm_joint_vel = norm_joint_vel * masks["joint_vel"]
        norm_actions = norm_actions * masks["previous_actions"]
        norm_ee_pos = norm_ee_pos * masks["ee_pos_ori"]
        norm_ee_camera_ori = norm_ee_camera_ori * masks["ee_pos_ori"]
        norm_left_finger_force = norm_left_finger_force * masks["contact_forces"]
        norm_right_finger_force = norm_right_finger_force * masks["contact_forces"]
        feature_rgb = feature_rgb * masks["rgb_feature"]
        feature_depth = feature_depth * masks["depth_feature"]
        image_clip_f = image_clip_f * masks["image_clip_feature"]
        ...
        combined_features = torch.cat(
            [feature1, feature2, feature_rgb, feature_depth, text_clip_f, image_clip_f, gumbel_masks], dim=-1
        )
