...
class FrankaCubeEnvCfg(DirectRLEnvCfg):
    ...
    camera_bottom =  CameraCfg(
        prim_path="/World/envs/env_.*/Robot/panda_link1/camera_bottom",
        update_period=0.1,
        height=480,
        width=640,
    ...
    camera_hand =  CameraCfg(
        prim_path="/World/envs/env_.*/Robot/panda_hand/camera_hand",
        update_period=0.1,
        height=480,
        width=640,
        data_types=["rgb", "distance_to_image_plane"],
...
class FrankaCubeEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        self._camera_bottom=Camera(self.cfg.camera_bottom)
        self._camera_hand = Camera(self.cfg.camera_hand)
        self.scene.sensors["camera_bottom"] = self._camera_bottom
        self.scene.sensors["camera_hand"] = self._camera_hand
    ...
    def _get_observations(self) -> dict:
        ...
        sensor: camera = self.scene.sensors["camera_bottom"]
        data = sensor.data.output["rgb"]
        # envs,,,channel
        output_tensor = F.interpolate(data.float().permute(0, 3, 1, 2), size=(120, 160), mode='bilinear',
                                      align_corners=False)
        output_tensor=output_tensor.permute(0, 3, 1, 2)[:,:3,:,:]/255
        model = models.resnet18(pretrained=False).to('cuda')
