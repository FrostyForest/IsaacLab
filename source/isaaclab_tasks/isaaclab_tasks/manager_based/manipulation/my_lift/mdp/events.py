...
from .string_target import PREDEFINED_TARGETS, TARGET_TO_ID, ID_TO_TARGET, NUM_TARGETS # 假设定义在一个单独的文件
...
def randomize_string_task_goal(env: LiftEnv, env_ids: torch.Tensor): # 类型提示为你的环境类
    ...
    if not hasattr(env, 'current_target_ids_per_env') or \
       not hasattr(env, 'encoded_task_goal_per_env') or \
       not hasattr(env, 'current_target_strings_per_env'):
        raise AttributeError("The environment object is missing required target attributes "
